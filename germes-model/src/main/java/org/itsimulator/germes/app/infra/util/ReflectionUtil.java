...
public class ReflectionUtil {
	...
	public static List<String> findSimilarFields(Class<?> clz1, Class<?> clz2) throws ConfigurationException {
		try {
			...
			List<String> targetFields = Stream.of(clz2.getDeclaredFields()).map((field) -> field.getName())
					.collect(Collectors.toList());
			return Stream.of(fields).map((field) -> field.getName()).filter((name) -> targetFields.contains(name))
					.collect(Collectors.toList());
